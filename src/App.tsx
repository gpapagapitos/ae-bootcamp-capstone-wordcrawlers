...
import type { LoadSuccess, RunSavePayload, SaveErrorReason } from './app/persistence/contracts.js';
...
import { REST_HEAL_AMOUNT, useProgressionStore } from './app/store/progressionStore.js';
...
export function App() {
  const { map, currentNodeId, visitedNodeIds, encounterHistory } = useMapStore();
  ...
  const [resumeCandidate, setResumeCandidate] = useState<LoadSuccess | null>(null);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [runOutcome, setRunOutcome] = useState<'victory' | 'defeat' | null>(null);
  ...
  const [actionToast, setActionToast] = useState<{ text: string; tone: 'good' | 'bad'; } | null>(null);
  ...
  const showActionToast = (text: string, tone: 'good' | 'bad' = 'good') => {
    ...
    toastTimeoutRef.current = window.setTimeout(() => setActionToast(null), 1600);
  };
  ...
  useEffect(() => {
    ...
    if (COMBAT_NODE_TYPES.has(selectedNode.type)) {
      ...
      startEncounter(currentNodeId, Date.now(), runDeck, heroId, boon, heroHp, heroMaxHp, buff);
      return;
    }
  ...
  const handleResumeRun = () => {
    ...
    handledNodeIds.current = new Set(resumeCandidate.payload.map.visitedNodeIds);
  ...
  if (encounterNodeId) {
    ...
    return <CombatHud encounterLabel={encounterLabel} onBackToMap={handleBackToMap} />;
  }
  ...
  if (!initialized && !titleAcknowledged) {
    return (
      <>
        ...
        />
        {confirmNewRun ? (
          <div className="resume-overlay" role="dialog" aria-modal="true" aria-labelledby="new-run-title">
            <div className="resume-card panel">
              <p className="eyebrow">New Run</p>
              <h2 id="new-run-title">Discard Your Saved Run?</h2>
              <p>Starting a new run will delete your existing saved progress. This cannot be undone.</p>
              <div className="resume-actions">
                <button type="button" className="ink-button" onClick={handleConfirmNewRunFromTitle}>
                  Discard &amp; Start New Run
                </button>
                <button type="button" className="ink-button" onClick={handleCancelNewRunFromTitle}>
                  Cancel
                </button>
              </div>
            </div>
    ...
    <>
      {loadError ? (
        <div className="save-alert" role="alert">
          <p>{loadError}</p>
          <button type="button" className="ink-button" onClick={handleAbandonRun}>
            Abandon Saved Run
          </button>
        </div>
      ) : null}
      ...
      {actionToast ? (
        <div className={`action-toast action-toast-${actionToast.tone}`} aria-live="polite">
          {actionToast.text}
        </div>
      ) : null}
      <MapScreen onReroll={handleRerollMap} onEndRun={handleRequestEndRun} />
      {activeModal === 'reward' ? (
        <RewardModal options={rewardOptions} onPick={pickReward} onClose={closeModal} />
      ) : null}
      {activeModal === 'shop' ? (
        <ShopModal
          boon={boon}
      ...
      {activeModal === 'rest' ? (
        <RestModal
          ...
          penaltyCardCount={runDeck.filter((card) => card.kind === 'penalty').length}
      ...
      {confirmEndRun ? (
        <div className="resume-overlay" role="dialog" aria-modal="true" aria-labelledby="end-run-title">
          <div className="resume-card panel">
            <p className="eyebrow">End Run</p>
            <h2 id="end-run-title">Return to Title Screen?</h2>
            <p>This will abandon your current run and any saved progress. This cannot be undone.</p>
            <div className="resume-actions">
              <button type="button" className="ink-button" onClick={handleConfirmEndRun}>
                End Run
              </button>
              <button type="button" className="ink-button" onClick={handleCancelEndRun}>
                Cancel
              </button>
            </div>
          </div>
      ...
      {runOutcome ? (
        <div className="resume-overlay" role="dialog" aria-modal="true" aria-labelledby="run-outcome-title">
          <div className="resume-card panel">
            <p className="eyebrow">Run Complete</p>
            <h2 id="run-outcome-title">{runOutcome === 'victory' ? 'Victory' : 'Defeat'}</h2>
            <p>
              {runOutcome === 'victory'
            ...
            <div className="resume-actions">
              <button type="button" className="ink-button" onClick={handleStartNewRun}>
                Start New Run
              </button>
            </div>
          </div>
