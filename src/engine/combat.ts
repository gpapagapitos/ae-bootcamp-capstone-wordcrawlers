...
function applyRelicEffect(state: RunState, def: ItemDef): void {
  switch (def.effectType) {
    ...
    case "gainEnergy":
      state.hero.energy = Math.min(
        MAX_ENERGY,
        state.hero.energy + def.effectValue,
      );
      break;
    case "applyHex":
  ...
  state.actionLog.push({
    turn: state.turn,
    message: `Relic ${def.name} triggers: ${def.description}`,
  });
}
...
function applyStageFlipIfDefeated(
  state: RunState,
...
function resolveEnemyIntent(state: RunState, intent: EnemyIntent): void {
  ...
  if (intent.type === "debuff") {
    ...
    state.actionLog.push({
      turn: state.turn,
      message:
        "Enemy debuff reduces your energy by 1 and slips a Penalty card into your deck.",
    });
    return;
  }
...
export function resolveClash(
  ...
  if (topCard?.ability) {
    state.actionLog.push({
      turn: state.turn,
      message: `${topCard.letter.toUpperCase()} ability: ${topCard.ability}`,
    });
  }
  ...
  if (topCard?.kind === "enemyVowel") {
    ...
    state.actionLog.push({
      turn: state.turn,
      message:
        "Enemy Vowel advances the enemy to its next intent and fatigues.",
    });
  }
