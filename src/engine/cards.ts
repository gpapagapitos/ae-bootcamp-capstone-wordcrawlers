...
export function deriveEdges(letter: string, value: number): { left: CardEdge; right: CardEdge } {
...
export function createLetterCard(id: string, letter: string, value: number, rarity: 1 | 2 | 3): Card {
  return {
    id,
    letter,
    value,
    ...
    return [
      {
        ...
        description: 'Passive: gain 1 block whenever you play a word without the Wild card.'
      }
    ];
  }
  ...
  return [
    {
      ...
      effectValue: 1,
      trigger: 'onStageFlip',
      description: 'Passive: apply 1 hex to the enemy the moment it flips to Stage 2.'
    }
  ];
}
