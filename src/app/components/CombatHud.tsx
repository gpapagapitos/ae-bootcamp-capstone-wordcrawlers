...
function CardFace({
  ...
  return (
    <>
      ...
      {compact ? (
        <span className="builder-slot-value">{formatEdge(card, splay)}</span>
      ) : null}
    </>
  );
}
...
export function CombatHud({ encounterLabel, onBackToMap }: CombatHudProps) {
  ...
  const [enemyBeatIntent, setEnemyBeatIntent] = useState<EnemyIntent | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (prev) {
      ...
      const heroBigHitThreshold = Math.max(
        6,
        Math.round(run.hero.maxHp * 0.15),
      );
      const enemyBigHitThreshold = Math.max(
        6,
        Math.round(run.enemy.maxHp * 0.15),
      );
  ...
  const intent =
    run.enemy.intents[run.enemy.intentIndex % run.enemy.intents.length] ?? null;
  ...
  const composedWord = composedCardIds
    ...
    .toUpperCase();
  const canCompose =
    run.phase === "spell" && !isVictory && !isDefeat && !resolving;
  const heroHpPct = Math.max(
    0,
    Math.min(100, (run.hero.hp / run.hero.maxHp) * 100),
  );
  const enemyHpPct = Math.max(
    0,
    Math.min(100, (run.enemy.hp / run.enemy.maxHp) * 100),
  );
  const heroFx = fx.filter((item) => item.target === "hero");
  const enemyFx = fx.filter((item) => item.target === "enemy");
  const showEnemyVowelSlot =
    run.enemyVowelAvailable || composedCardIds.includes(ENEMY_VOWEL_CARD_ID);
  const totalHandSlots =
    1 + (showEnemyVowelSlot ? 1 : 0) + run.deck.hand.length;
  const needsWildLetter =
    composedCardIds.includes(WILD_CARD_ID) && !/^[a-z]$/i.test(wildLetter);
  ...
  const resolvingCardId =
    resolving && resolveIndex >= 0 ? composedCardIds[resolveIndex] : null;
  const resolvingCard = resolvingCardId
    ? run.deck.hand.find((item) => item.id === resolvingCardId)
    : null;
  const resolvingEdge = resolvingCard ? getEdge(resolvingCard, splay) : null;
  const wordIsValid =
    composedCardIds.length > 0 &&
    !needsWildLetter &&
    validateWordStrict(composedWord).valid;
  ...
  return (
    <div className="screen-root combat-layout">
      ...
      <section
        className={`battlefield-shell ${castPulse ? "arena-cast" : ""} ${clashPulse ? "arena-clash" : ""} ${impactShake ? "arena-impact" : ""}`}
      >
          ...
          <div className="arena-scene">
            {resolving && (runningTotal.hits > 0 || runningTotal.blocks > 0) ? (
              <div className="arena-total-damage" key={`total-${resolveIndex}`}>
                <span className="arena-total-damage-value">
                  {runningTotal.hits > 0
                    ? runningTotal.hits
                    : runningTotal.blocks}
                </span>
                <span className="arena-total-damage-icon" aria-hidden="true">
                  {runningTotal.hits > 0 ? "⚔" : "🛡"}
                </span>
              </div>
            ) : null}
            {enemyBeatIntent ? (
              <div className="enemy-beat-banner" role="status">
                <span className="enemy-beat-banner-icon" aria-hidden="true">
                  {INTENT_ICONS[enemyBeatIntent.type]}
                </span>
                <span className="enemy-beat-banner-text">
                  Enemy Turn — {formatIntent(enemyBeatIntent)}
                </span>
              </div>
            ) : null}
            <div className="arena-combatants">
              <div
                className={`combatant combatant-hero ${heroShake ? "is-hit" : ""} ${run.phase === "spell" ? "is-active" : ""}`}
              >
                {heroFx.map((item) => (
                  <span
                    key={item.id}
                    className={`floating-fx floating-fx-${item.kind}`}
                  >
                    {item.text}
                  </span>
                ))}
                <span className="combatant-banner combatant-banner-hero">
                  Hero
                </span>
                ...
                <div
                  className={`stat-bar hp-bar ${heroHpPct <= 25 ? "low" : heroHpPct <= 50 ? "mid" : ""}`}
                  role="progressbar"
                  aria-label="Hero HP"
                  aria-valuenow={run.hero.hp}
                  aria-valuemax={run.hero.maxHp}
                >
                  <div
                    className="stat-bar-trail"
                ...
                {run.hero.block > 0 ? (
                  <span className="combatant-stat combatant-block">
                    🛡 {run.hero.block}
                  </span>
                ) : null}
                <span className="combatant-stat">⚡ {run.hero.energy} EN</span>
                {run.hero.hex > 0 ? (
                  <span className="combatant-stat combatant-hex">
                    ☣ {run.hero.hex} HEX
                  </span>
                ) : null}
                {run.hero.boon > 0 ? (
                  <span className="combatant-stat combatant-boon">
                    ✦ {run.hero.boon} BOON
                  </span>
                ) : null}
              </div>
              ...
              <div
                className={`combatant combatant-enemy ${enemyShake ? "is-hit" : ""} ${run.phase === "clash" ? "is-active" : ""}`}
              >
                {enemyFx.map((item) => (
                  <span
                    key={item.id}
                ...
                <span className="combatant-banner combatant-banner-enemy">
                  <span className="combatant-banner-title">
                    {run.enemy.name} · Stage {run.enemy.stage}
                  </span>
                </span>
                <div className="combatant-portrait" aria-hidden="true">
                  <span className="portrait-glyph">☠</span>
                ...
                <div
                  className={`stat-bar hp-bar ${enemyHpPct <= 25 ? "low" : enemyHpPct <= 50 ? "mid" : ""}`}
                  role="progressbar"
                  aria-label="Enemy HP"
                  aria-valuenow={run.enemy.hp}
                  aria-valuemax={run.enemy.maxHp}
                >
                  <div
                    className="stat-bar-trail"
                ...
                {intent ? (
                  <div
                    className={`enemy-next-action enemy-next-action-${intent.type}`}
                    data-tutorial="intent-row"
                    aria-label="Enemy's next action"
                  >
                    <span className="enemy-next-action-label">Next</span>
                    <span className="enemy-next-action-icon" aria-hidden="true">
                      {INTENT_ICONS[intent.type]}
                    </span>
                    <span className="enemy-next-action-text">
                      {formatIntent(intent)}
                    </span>
                  </div>
                ) : null}
                <div className="arena-pill-row" aria-label="Upcoming intents">
                  {intentPreview.map((item) => (
                    <span
                      ...
                      className={
                        item.offset === 0 ? "arena-pill active" : "arena-pill"
                      }
                      title={formatIntent(item.intent)}
                    >
                      <span aria-hidden="true">
                        {INTENT_ICONS[item.intent.type]}
                      </span>{" "}
                      {formatIntentValue(item.intent)}
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div
              className="battlefield-ground"
              aria-label={`Composed word: ${composedWord || "empty"}`}
            >
              {composedCardIds.length === 0 ? null : (
                <div className="builder-slots">
                  {composedCardIds.map((cardId, index) => {
                    const handCard = run.deck.hand.find(
                      (item) => item.id === cardId,
                    );
                    const letter =
                      cardId === WILD_CARD_ID
                    ...
                    if (handCard) {
                      return (
                        ...
                        >
                          {resolveIndex === index ? (
                            <span
                              className="resolve-edge-callout"
                              aria-hidden="true"
                            >
                              {formatEdge(handCard, splay)}
                            </span>
                          ) : null}
                    ...
                    if (cardId === WILD_CARD_ID) {
                      return (
                        <div
                          key={cardId}
                          className="builder-slot builder-slot-wild"
                          style={{ animationDelay: `${index * 40}ms` }}
                        >
                          <input
                            type="text"
                            ...
                            disabled={!canCompose}
                            onChange={(event) =>
                              setWildLetter(event.target.value)
                            }
                    ...
                    return (
                      <button
                        ...
                        title="Remove from word"
                      >
                        <span className="builder-slot-letter">
                          {letter.toUpperCase()}
                        </span>
                      </button>
                    );
              ...
              <div className="word-totals" aria-label="Word totals">
                {(resolving ? runningTotal.hits : wordTotals.hits) > 0 ? (
                  <span
                    key={`hits-${resolving ? runningTotal.hits : wordTotals.hits}`}
                    className="word-total word-total-hits"
                  >
                    ⚔ {resolving ? runningTotal.hits : wordTotals.hits}
                  </span>
                ) : null}
                {(resolving ? runningTotal.blocks : wordTotals.blocks) > 0 ? (
                  <span
                    key={`blocks-${resolving ? runningTotal.blocks : wordTotals.blocks}`}
                    className="word-total word-total-blocks"
                  >
                    🛡 {resolving ? runningTotal.blocks : wordTotals.blocks}
                  </span>
                ) : null}
                {(resolving ? runningTotal.energy : wordTotals.energy) > 0 ? (
                  <span
                    key={`energy-${resolving ? runningTotal.energy : wordTotals.energy}`}
                    className="word-total word-total-energy"
                  >
                    ⚡ {resolving ? runningTotal.energy : wordTotals.energy}
                  </span>
                ) : null}
              </div>
            </div>
        ...
        <section
          className="hand-tray"
          data-tutorial="hand-tray"
          aria-label="Hand tray"
        >
          <div className="hand-header">
            <div className="hand-header-left">
              ...
              <div className="deck-piles" aria-label="Deck status">
                <div
                  className={`pile pile-draw ${run.deck.draw.length === 0 ? "pile-empty" : ""}`}
                  title={`Draw pile: ${run.deck.draw.length} cards`}
                >
                ...
                <div
                  className={`pile pile-discard ${run.deck.discard.length === 0 ? "pile-empty" : ""}`}
                  title={`Discard pile: ${run.deck.discard.length} cards`}
                >
                  <div className="pile-stack" aria-hidden="true">
                    <span className="pile-card" />
                    <span className="pile-card" />
                    ...
                    <span className="pile-count">
                      {run.deck.discard.length}
                    </span>
                  </div>
                  <span className="pile-label">Discard</span>
                </div>
                <div
                  className={`pile pile-fatigue ${run.deck.fatigue.length === 0 ? "pile-empty" : ""}`}
                  title={`Fatigue pile: ${run.deck.fatigue.length} cards`}
                >
                  <div className="pile-stack" aria-hidden="true">
                    <span className="pile-card" />
                    <span className="pile-card" />
            ...
            <div
              className="splay-toggle"
              data-tutorial="splay-toggle"
              role="group"
              aria-label="Splay direction"
            >
              <button
                type="button"
          ...
          <div className="hand-cards" role="list">
            ...
            {showEnemyVowelSlot ? (
              ...
              >
                <p className="hand-letter">
                  {(run.enemy.weakVowel ?? "?").toUpperCase()}
                </p>
            ...
            {run.deck.hand.map((card, cardIndex) => (
              <button
                ...
                role="listitem"
                style={fanStyle(
                  (showEnemyVowelSlot ? 2 : 1) + cardIndex,
                  totalHandSlots,
                )}
                disabled={!canCompose}
        ...
        <section className="combat-controls" aria-label="Combat controls">
          <div
            className="hero-abilities-panel"
            data-tutorial="items-row"
              ...
              {run.items.map((item) => (
                <button
                  ...
                  disabled={
                    ...
                    item.spent ||
                    item.usedThisTurn ||
                    run.hero.energy < item.def.energyCost
                  }
                  ...
                  onClick={() => {
                    ...
                    window.setTimeout(
                      () =>
                        setCastingItemId((current) =>
                          current === item.def.id ? null : current,
                        ),
                      420,
                    );
                    useItem(item.def.id);
                ...
                >
                  ...
                  <span className="item-button-effect">
                    {formatItemEffect(
                      item.def.effectType,
                      item.def.effectValue,
                    )}
                    <span className="item-button-cost">
                      ⚡{item.def.energyCost}
                    </span>
                  </span>
                </button>
          ...
          <button
            ...
            disabled={
              !canCompose || composedCardIds.length === 0 || needsWildLetter
            }
            title={
              needsWildLetter
                ? "Choose a letter for the Wild card first"
                : undefined
            }
          >
        ...
        {lastError ? (
          <section className="panel combat-error" role="alert">
            <p>{lastError}</p>
            <button className="ink-button" onClick={clearError}>
              Dismiss
            </button>
          </section>
        ) : null}
        ...
        {isVictory || isDefeat ? (
          <div className="combat-outcome-overlay" aria-live="polite">
            <section
              className={`panel combat-outcome-card ${isVictory ? "is-victory" : "is-defeat"}`}
            >
              <span className="combat-outcome-banner">
                {isVictory ? "Victory" : "Defeat"}
              </span>
              <span className="combat-outcome-glyph" aria-hidden="true">
                {isVictory ? "\u2694" : "\u2620"}
              </span>
              <p>
                {isVictory
