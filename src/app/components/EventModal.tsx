...
const CARD_EFFECT_LABELS: Record<
  NonNullable<EventChoice["cardEffect"]>,
  string
> = {
  addCard: "Card Added",
...
function EventOutcomeStats({ effects }: { effects: EventChoice | null }) {
  ...
  if (effects.deltaHp) {
    rows.push({
      icon: "❤",
      text: `${effects.deltaHp > 0 ? "+" : ""}${effects.deltaHp} HP`,
      tone: effects.deltaHp > 0 ? "good" : "bad",
    });
  }
  if (effects.deltaBoon) {
    rows.push({
      icon: "✦",
      text: `${effects.deltaBoon > 0 ? "+" : ""}${effects.deltaBoon} Boon`,
      tone: effects.deltaBoon > 0 ? "good" : "bad",
    });
  }
  if (effects.cardEffect) {
    rows.push({
      icon: "❖",
      text: CARD_EFFECT_LABELS[effects.cardEffect],
      tone: effects.cardEffect === "curseCard" ? "bad" : "good",
    });
  }
  if (effects.buff) {
    rows.push({
      icon: "⚡",
      text: describeBuff(effects.buff),
      tone: "neutral",
    });
  }
  ...
  return (
    <div className="event-outcome-stats">
      {rows.map((row) => (
        <div
          key={row.text}
          className={`event-outcome-stat event-outcome-stat-${row.tone}`}
        >
          <span className="event-outcome-stat-icon" aria-hidden="true">
            {row.icon}
          </span>
          <span>{row.text}</span>
        </div>
...
export function EventModal({
  ...
  const choices = event.choices.filter(
    (choice) => !choice.heroOnly || choice.heroOnly === heroId,
  );
        ...
        {result ? (
          ...
          <ul className="event-scroll-choices">
            {choices.map((choice) => (
              <li key={choice.id}>
                <button
                  className="event-scroll-choice"
                  onClick={() => onChoose(choice.id)}
                >
                  <span>{choice.label}</span>
                  <span
                    className="event-scroll-choice-arrow"
                    aria-hidden="true"
                  >
                    ➤
                  </span>
