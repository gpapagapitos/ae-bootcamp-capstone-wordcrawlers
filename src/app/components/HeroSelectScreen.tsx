...
export function HeroSelectScreen({ onSelect }: HeroSelectScreenProps) {
  ...
  const showPrev = () => setIndex((current) => (current - 1 + HERO_OPTIONS.length) % HERO_OPTIONS.length);
  const showNext = () => setIndex((current) => (current + 1) % HERO_OPTIONS.length);
      ...
      <section className="hero-showcase">
        ...
        <div className="hero-showcase-info">
          ...
          <div className="hero-showcase-stats">
            <span className="hero-stat">&#10084;&#65039; HP {hero.hp}/{hero.hp}</span>
            <span className="hero-stat">&#9889; Energy {hero.energy}</span>
            <span className="hero-stat">&#128292; {hero.starterLetters.length} Starter Letters</span>
          </div>

          <div className="hero-card-letters">
            {hero.starterLetters.map((letter, letterIndex) => (
              <span key={`${hero.id}-${letterIndex}-${letter}`} className="hero-card-letter">{letter}</span>
            ))}
          </div>
          ...
          <div className="hero-showcase-abilities">
            ...
            {coreItems.map((item) => (
              <div key={item.id} className="hero-ability-slot">
                <span className="hero-ability-icon" aria-hidden="true">{iconForEffect(item.effectType)}</span>
                <div className="hero-ability-body">
                  <div className="hero-ability-name-row">
                    <p className="hero-ability-name">{item.name}</p>
                    <span className="hero-ability-cost">&#9889; {item.energyCost}</span>
                  </div>
          ...
          <button className="ink-button ink-button-primary hero-embark" onClick={() => onSelect(hero.id)}>
            Begin Journey
          </button>
        </div>
