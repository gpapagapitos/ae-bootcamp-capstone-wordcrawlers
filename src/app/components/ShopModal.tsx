...
export function ShopModal({ boon, offers, deck, onBuy, onRemove, onClose }: ShopModalProps) {
  const [replaceTargetId, setReplaceTargetId] = useState<string | null>(null);
  const replaceableDeck = deck.filter((card) => card.kind !== 'penalty');

  return (
    <div className="modal-overlay" role="presentation">
      <section className="panel modal-panel" role="dialog" aria-modal="true" aria-label="Shop">
        <header className="modal-header modal-header-banner">
          <h2>Ink Merchant</h2>
          <p>Spend boons to replace a deck card with a new one, or remove one from your deck.</p>
        ...
        <div className="shop-layout">
          ...
          <div>
            <h3>Your Deck</h3>
            <p className="shop-hint">Select a card to replace, or spend 5 boons to remove one outright.</p>
            <ul className="deck-list">
              {replaceableDeck.slice(0, 12).map((card) => (
                <li key={card.id} className={`deck-list-item${replaceTargetId === card.id ? ' deck-list-item-selected' : ''}`}>
                  <button
                    type="button"
                    className="deck-list-select"
                    onClick={() => setReplaceTargetId(replaceTargetId === card.id ? null : card.id)}
                    aria-pressed={replaceTargetId === card.id}
                  >
                    <span className="deck-list-letter">{card.letter.toUpperCase()}</span>
                    <span className="deck-list-info">
                      <strong>{getCardDisplayName(card)}</strong>
                      <span className="deck-list-value">Value {card.value}</span>
                    </span>
                  </button>
                  <button
        ...
        <footer className="modal-footer">
          <button className="ink-button" onClick={onClose}>Leave Shop</button>
        </footer>
      </section>
    </div>
  );
