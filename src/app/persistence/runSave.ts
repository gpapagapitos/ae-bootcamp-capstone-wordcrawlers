import type { LocalStorageLike, LoadResult, RunSavePayload, SaveResult, SaveSlot } from './contracts.js';
import { RUN_SAVE_KEYS } from './contracts.js';
import { createEnvelope, toLoadFailure, toLoadSuccess, validateEnvelope } from './validate.js';
...
function readSlot(storage: LocalStorageLike, slot: SaveSlot): LoadResult {
  const key = slot === 'current' ? RUN_SAVE_KEYS.current : RUN_SAVE_KEYS.previous;
...
export function saveRunSnapshot(payload: RunSavePayload, storage?: LocalStorageLike): SaveResult {
...
export function loadRunSnapshot(storage?: LocalStorageLike): LoadResult {
  ...
  return toLoadFailure(current.reason !== 'not-found' ? current.reason : previous.reason);
}
