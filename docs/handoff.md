# Evolving Handoff

This document is session-friendly and should be updated frequently.

## How To Update

1. Update tasks via board script commands.
2. Render board markdown.
3. Append a new handoff snapshot.

Commands:

- npm run board:render
- npm run board:check
- npm run handoff:snapshot

## Snapshot Log

(Entries are appended by script.)

### Snapshot 2026-07-31T17:03:15.881Z

- Backlog: 2
- In Progress: 1
- Blocked: 0
- Done: 0

- In-flight focus:
  - WC-001: Build combat HUD with hand, intent, and action log (in-progress)

### Snapshot 2026-07-31T17:06:38.544Z

- Backlog: 2
- In Progress: 1
- Blocked: 0
- Done: 1

- In-flight focus:
  - WC-001: Build combat HUD with hand, intent, and action log (in-progress)

### Snapshot 2026-07-31T17:09:32.969Z

- Backlog: 2
- In Progress: 1
- Blocked: 0
- Done: 1

- In-flight focus:
  - WC-001: Build combat HUD with hand, intent, and action log (in-progress)

### Snapshot 2026-07-31T17:10:47.522Z

- Backlog: 2
- In Progress: 1
- Blocked: 0
- Done: 1

- In-flight focus:
  - WC-001: Build combat HUD with hand, intent, and action log (in-progress)

### Snapshot 2026-07-31T17:15:44.425Z

- Backlog: 1
- In Progress: 0
- Blocked: 0
- Done: 3

- In-flight focus:
  - none

### Snapshot 2026-07-31T17:16:47.936Z

- Backlog: 11
- In Progress: 0
- Blocked: 0
- Done: 3

- In-flight focus:
  - none

### Snapshot 2026-07-31T17:18:00.310Z

- Backlog: 10
- In Progress: 0
- Blocked: 0
- Done: 4

- In-flight focus:
  - none

### Snapshot 2026-07-31T17:23:31.365Z

- Backlog: 7
- In Progress: 0
- Blocked: 0
- Done: 7

- In-flight focus:
  - none

### Snapshot 2026-07-31T17:27:58.315Z

- Backlog: 7
- In Progress: 0
- Blocked: 0
- Done: 7

- In-flight focus:
  - none

### Snapshot 2026-07-31T17:27:59.986Z

- Backlog: 7
- In Progress: 0
- Blocked: 0
- Done: 7

- In-flight focus:
  - none

### Snapshot 2026-08-03T15:28:53.483Z

- Backlog: 7
- In Progress: 0
- Blocked: 0
- Done: 8

- In-flight focus:
  - none
