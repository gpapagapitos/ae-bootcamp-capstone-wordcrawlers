...
describe("turn flow", () => {
  ...
  it("applies 1 HP loss for an unplayed Penalty card left in hand at Cleanup (spec 009 R6)", () => {
    ...
    expect(
      state.actionLog.some((entry) =>
        entry.message.includes("Unplayed Penalty card"),
      ),
    ).toBe(true);
  });
  ...
  it("returns a played Penalty card to the shared pool instead of discard/fatigue", () => {
    ...
    expect(state.deck.discard.some((card) => card.id === "penalty-a")).toBe(
      false,
    );
    expect(state.deck.fatigue.some((card) => card.id === "penalty-a")).toBe(
      false,
    );
  });
