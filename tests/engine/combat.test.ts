...
describe('resolveTopCard', () => {
  it('picks the first card for splay right', () => {
    const cards = [makeCard('a', 'a', 1), makeCard('r', 'r', 2), makeCard('c', 'c', 2)];
  ...
  it('skips wild cards when finding the top card', () => {
    const wild: Card = { id: 'w', letter: '*', value: 0, rarity: 1, tags: [], kind: 'wild' };
    expect(resolveTopCard([wild, makeCard('c', 'c', 2)], 'right')?.id).toBe('c');
  });
});
...
describe('resolveClash', () => {
  ...
  it('fatigues only the top card, not the whole word', () => {
    ...
    state.pendingWordCards = [makeCard('a', 'a', 1), makeCard('r', 'r', 2), makeCard('c', 'c', 2)];
  ...
  it('grants no bonus energy when the word uses the Wild card', () => {
    const state = createInitialRunState(42, 'duelist');
    const wild: Card = { id: 'wild-duelist', letter: 'a', value: 0, rarity: 1, tags: [], kind: 'wild' };
    state.pendingWord = 'arc';
    state.pendingWordCards = [wild, makeCard('r', 'r', 2), makeCard('c', 'c', 2)];
  ...
  it('advances the enemy intent and fatigues the Enemy Vowel card when it is top (spec 009 R5)', () => {
    ...
    expect(state.enemy.intentIndex).toBe((startingIndex + 1) % state.enemy.intents.length);
  ...
  it('flips the enemy to Stage 2 with overflow damage and grants a Penalty card', () => {
    ...
    state.pendingWord = 'arc';
    state.pendingWordCards = [makeCard('a', 'a', 5), makeCard('r', 'r', 2), makeCard('c', 'c', 2)];
    ...
    expect(state.deck.discard.some((card) => card.kind === 'penalty')).toBe(true);
  });
